...
def bof_address_assign(ir):
    ...
    while True:
        try:
            ...
            available_ips = []
            for i in ip_range:
                ret = os.system(f'ping -o -c 2 -W 3000 {i}')
                if ret != 0:
                    a_ip = str(i)
                    available_ips.append(a_ip)
            if len(available_ips) < num_rtr[-1]:
            ...
            r_ipdict = {num_rtr[i]: available_ips[i]
                        for i in range(len(num_rtr))}

            for k, v in r_ipdict.items():
                print(f"Rotuer{k} has a management IP address of: {v}")
            print(f'Your default gateway is {dg}')
            break
        except ValueError:
